<template>
  <div class="app-container">
    <div class="filter-container" style="margin-bottom: 20px;margin-left: 50px" >
      ...
      <el-button v-show="$route.meta.add" class="filter-item" style="margin-left: 10px;" type="primary" icon="el-icon-edit" @click="dialogFormVisible = true">添加角色</el-button>
    </div>
...
<script>
import { getList, add, delectByid, selectRolesByid } from '@/api/roles'
...
export default {
  ...
  methods: {
    ...
    add() {
      add(this.form.id, this.form.name)
        .then(res => {
          this.dialogFormVisible = false
          this.form.name = ''
          this.form.id = ''
